      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6 py-8 md:py-16">
        ...
        <div className="mb-24 md:mb-32" ref={journeyRef}>
          ...
          <div className="relative max-w-6xl mx-auto">
            ...
            <div className="absolute left-4 md:left-8 top-0 bottom-0 w-0.5 md:w-1 bg-gray-700 rounded-full"></div>
            <div
              className="absolute left-4 md:left-8 top-0 w-0.5 md:w-1 bg-gradient-to-b from-blue-500 via-purple-500 to-pink-500 rounded-full transition-all duration-300 ease-out"
              style={{ height: `${scrollProgress * 100}%` }}
            ></div>

            {/* Enhanced Moving Ball */}
            <div
              className="absolute left-2 md:left-6 w-4 h-4 md:w-5 md:h-5 bg-white rounded-full shadow-lg border-2 md:border-4 border-blue-500 transition-all duration-300 ease-out z-20"
              style={{
            ...
            <div className="space-y-12 md:space-y-16">
              {journeyItems.map((item, index) => (
                <div key={index} className="relative group">
                  ...
                  <div className="absolute left-2 md:left-4 top-8 md:top-12 w-6 h-6 md:w-8 md:h-8 bg-gray-900 border-2 md:border-4 border-gray-600 rounded-full z-10 group-hover:border-white transition-all duration-300">
                    <div className={`absolute inset-0.5 md:inset-1 bg-gradient-to-r ${item.gradient} rounded-full`}></div>
                  </div>

                  {/* Enhanced Content Card */}
                  <div className="ml-12 md:ml-20 bg-gray-800/30 backdrop-blur-sm rounded-xl md:rounded-2xl p-4 md:p-6 lg:p-8 border border-gray-700/50 transition-all duration-300 group-hover:shadow-2xl group-hover:border-gray-600/70 group-hover:bg-gray-800/40">
                    {/* Enhanced Header */}
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 md:gap-6 mb-4 md:mb-6">
                      <div className="flex items-start gap-3 md:gap-4">
                        <div className={`rounded-full p-2 md:p-3 bg-gradient-to-br ${item.gradient} text-white shadow-md flex-shrink-0`}>
                          {item.icon}
                        </div>
                        <div className="flex-1">
                          <div className="text-xs md:text-sm text-gray-400 mb-1">{item.date}</div>
                          <h3 className="text-lg md:text-xl lg:text-2xl font-bold text-white mb-1">
                            {item.title}
                          </h3>
                          <div className="text-sm md:text-base text-blue-300 mb-3 md:mb-4">{item.organization}</div>
                          ...
                          <div className="flex flex-wrap gap-2 mb-4">
                            {item.achievements.map((achievement, achIndex) => (
                              <span
                                key={achIndex}
                                className={`px-2 py-1 md:px-3 md:py-1 bg-gradient-to-r ${item.gradient} text-white text-xs md:text-sm rounded-full  shadow-sm`}
                              >
                                {achievement}
                              </span>
                            ))}
                    ...
                    <div className="space-y-2 md:space-y-3">
                      {item.description.map((point, pointIndex) => (
                        <div key={pointIndex} className="flex items-start gap-2 md:gap-3">
                          <ChevronRight className="w-4 h-4 md:w-5 md:h-5 text-blue-400 mt-0.5 flex-shrink-0" />
